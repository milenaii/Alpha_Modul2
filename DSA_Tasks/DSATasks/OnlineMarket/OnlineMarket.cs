...
namespace OnlineMarket
{
    public class Program
    {
        static void Main()
        {
            ...
            List<Product> list = new List<Product>();  //filter by price 2 %
                    ...
                    case "filter":
                        ...
                        else
                        {
                            if (parameters.Length > 5 && parameters[3] == "from" && parameters[5] == "to")
                            {
                                ...
                                var filtered = list.Where(x => x.Price >= fromPrice && x.Price <= toPrice).OrderBy(x => x.Price).Take(10);
                            ...
                            else if (parameters[3] == "from")
                            {
                                ...
                                var filteredTo = list.Where(x => x.Price > minPrice).OrderBy(x => x.Price).Take(10);
                            ...
                            else if (parameters[3] == "to")
                            {
                                ...
                                var filteredTo = list.Where(x => x.Price < maxPrice).OrderBy(x => x.Price).Take(10);
                                string filterTo = string.Format("Ok: {0}", string.Join(", ", filteredTo));
                                filterTo.TrimEnd(',', ' ');
                                Console.WriteLine(filterTo);
                            }
